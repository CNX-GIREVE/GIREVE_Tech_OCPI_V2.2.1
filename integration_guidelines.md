...
GIREVE requires all endpoints to be in HTTPS in order to secure communication between the two backends.
...
However, only the client certificate used by the partner to send requests to IOP, which is generated by Gireve, is mandatory. The client certificate used by IOP to send requests to the partner, provided by the partner, is optional.
...
o	Locations module: 20 objects per page
